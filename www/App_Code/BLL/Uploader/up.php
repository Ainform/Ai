...
if (!session_is_registered(BLL_Uploader_Image::IndexUploadPath))
	die('<?xml version="1.0" encoding="utf-8"?><root>Error 3</root>');
...
foreach ($images as $image)
{
	...
	{
		Helpers_LogHelper::AddLogEntry('Image Upp '.$fileName);

		// обновляем позиции
		$order = new DAL_ArrayOrder($images, 'Order');
		$imageList = $order->UpRecord($image['Order']);
...
die('<?xml version="1.0" encoding="utf-8"?><root>File '.($upped ? '' : 'Not').' Upped</root>');
