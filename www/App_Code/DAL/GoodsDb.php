...
/**
 * DAL_GoodsDb class
 * Класс для работы с товарами в БД
 *
 * @author Frame
 * @version GoodsDb.class.php, v 1.0.1
 * @copyright (c) by VisualDesign
 */
class DAL_GoodsDb extends DAL_BaseDb {
    ...
    function GetGoodLink($params) {
        ...
        $secDb->GetSectionFullPath($good['SectionId'], $sectionpath);
        $section = $secDb->GetSection($good['SectionId']);
        ...
        return $shopname . $sectionpath . $this->GetGoodAlias($id) . '/';
    }
    ...
    public function GetAllFromSectionAndSubsection($sectionId, $page=0, $count=10) {
        $secDb = new DAL_SectionsDb;
        $tree = $secDb->GetTree(null, $sectionId);
        //wtf($tree,false);
        unset($secDb);
        $goods = array();
    ...
    public function GetCrossPrice($Price, $PrevCurrency, $CurCurrency) {
        ...
        if (count($result) > 0)
            return round($Price * $result[0]['Value'], 2);
        else
            return false;
    }
