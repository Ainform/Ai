...
function smarty_function_AnalyticFilesUploader($params, &$smarty)
{
	...
	$html .= '<thead><tr><th align="center" colspan="5">Файлы</th></tr></thead><tbody id="'.$id.'_tbody">';
	...
	if (isset($files))
	{
		foreach ($files as $file)
		{
			...
			// кнопка удаление
			$html .= '<td width="16"><input type="image" name="handlerBtnDel:'.$file['id'].'" src="/img/admin/close_16.png" alt="Удаление файла" height="16" width="16" onclick="delUplFile(\''.$file['id'].'\'); return false;" /></td>';

			// картинка файла
			$html .= '<td><img style="vertical-align:middle" src="/img/admin/document_blank.png" alt="'.$file['title'].'" title="'.$file['title'].'" />'.$file['filename'].'</td>';

			// кнопка поднятия
			$html .= '<td width="16"><input type="image" name="handlerBtnUp:'.$file['id'].'" src="/img/admin/arrow_up_16.png" alt="Поднять файл" height="16" width="16" onclick="upUplFile(\''.$id.'\', \''.$file['id'].'\'); return false;" /></td>';

			// кнопка опускания изображения
			$html .= '<td width="16"><input type="image" name="handlerBtnDown:'.$file['id'].'" src="/img/admin/arrow_down_16.png" alt="Опустить изображение" height="16" width="16" onclick="downUplFile(\''.$id.'\', \''.$file['id'].'\'); return false;" /></td>';
