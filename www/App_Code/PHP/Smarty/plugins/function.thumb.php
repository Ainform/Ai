...
/*function TrueColor2Palette($image, $dither, $ncolors) {
	$width = imagesx( $image );
...
function smarty_function_thumb($params, &$smarty)
	...
	if (empty($params['hint'])) $params['hint'] = false;
...
//	if($_DST['width'] <= 150 && $_DST['height'] <= 150) $_DST['type'] = 3;
	...
	if (empty($params['html'])) $_RETURN['img'] = '<img src="'.$_DST['file'].'" '.$_DST['string'].' alt="Thumb" title="" />';
	...
	if($_SRC['type'] == 3 || $_SRC['type'] == 1)
	{
		$background = imagecolorallocatealpha($_DST['image'], 0, 0, 0, 127);
		ImageColorTransparent($_DST['image'], $background);
	...
	if (!empty($params['sharpen']) && $params['sharpen'] != "false") $_DST['image'] = UnsharpMask($_DST['image'],80,.5,3);
