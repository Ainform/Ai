...
{if isset($Data.error)}<div class="message">{$Data.error}</div>{/if}
...
<table>
    <tr><td style="width:150px;"><label for="fio">ФИО</label></td>
        <td><input type="text" id="recipient_fio" name="recipient_fio" value="{$smarty.session.recipient_fio}"/></td></tr>
    <tr><td style="width:150px;"><label>Телефон</label></td>
        <td><input type="text" id="recipient_phone" name="recipient_phone" value="{$smarty.session.recipient_phone}"/></td></tr>
</table>
...
{$smarty.session.address_full}
            </textarea></td></tr>
    <tr><td style="width:150px;">Дата доставки</td>
        <td><input type="text" id="address_date" name="address_date" value="{$smarty.session.address_date}"/></td></tr>
    <tr><td style="width:150px;">Время доставки</td>
        <td><input type="text" id="address_time" name="address_time" value="{$smarty.session.address_time}"/></td></tr>
</table>

<h1>Ваши контактные данные*<a name="contact_a"></a></h1>
<table style="width:50%">
    <tr><td style="width:150px;">ФИО</td>
        <td><input type="text" id="contact_fio" name="contact_fio" value="{$smarty.session.contact_fio}"/></td></tr>
    <tr><td style="width:150px;">Телефон</td>
        <td><input type="text" id="contact_phone" name="contact_phone" value="{$smarty.session.contact_phone}"/></td></tr>
    <tr><td style="width:150px;">Емейл</td>
        <td><input type="text" id="contact_email" name="contact_email" value="{$smarty.session.contact_email}"/></td></tr>
    <tr><td style="width:150px;">Дата рождения</td>
        <td><input type="text" id="contact_birthdate" name="contact_birthdate" value="{$smarty.session.contact_birthdate}"/></td></tr>
</table>
<p> * обязательны для заполнения при оформлении карты почетного гостя</p>
<p><input type="checkbox" name="discounts_and_actions" id="discounts_and_actions" {if isset($smarty.session.discounts_and_actions)}checked="checked"{/if}/>
    <label for="discounts_and_actions">Хочу получать информацию о скидках и акциях</label></p>
<p><input type="checkbox" name="card" id="card" {if isset($smarty.session.card)}checked="checked"{/if}/>
    <label for="card">Оформить карту почётного гостя</label></p>
<p><input type="checkbox" name="delivery" id="delivery" {if isset($smarty.session.delivery)}checked="checked"{/if}/>
