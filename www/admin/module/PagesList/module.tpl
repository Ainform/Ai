        ...
        $("#btnAdd").click(function(){
            ...
            if(node){
                ...
                $.ajax({
                    type: "POST",
                    url: "{$PageAddress}",
                    data: "handlerBtnNew=handlerBtnNew&moduleId={$Module->moduleId}&parentId=0&title=Новый раздел",
                    success: function(url){
            ...
            if (confirm('Удалить раздел? Все вложенные разделы будут удалены безвозвратно.'))
            {
                if(node){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{$PageAddress}",
                        data: "handlerBtnDelete=handlerBtnDelete&moduleId={$Module->moduleId}&pageId="+node.data.id,
                        success: function(msg){
                            //alert( "Data Saved: " + msg );
                        }
                    });
